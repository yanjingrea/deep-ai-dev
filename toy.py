...
if selected == "Search":
    ...
    if loc_select == 'City':
        city_select = st.selectbox(label='city', options=['City'] + list(master_city['CITYSTATE'].unique()),
                                   label_visibility='collapsed')
        st.caption(
            'Note: City is aggregated to the USPS designation which may include additional nearby cities/towns/municipalities')
        zip_select = 'Zip'
    if loc_select == 'Zip':
        zip_select = st.selectbox(label='zip', options=['Zip'] + list(master_zip['ZIP'].unique()),
                                  label_visibility='collapsed')
    ...
    with st.expander('Advanced Settings'):
        ...
        count_select = col2.number_input(label='How many similar locations returned? (5-25)', min_value=5, max_value=25,
                                         value=10, step=5)
    ...
    if loc_select == 'City':
        if city_select != 'City':
            ...
            EnvironmentCols_sc = ['PCT_WATER_SC', 'ENV_INDEX_SC', 'PCT_TOPARK_ONEMILE_SC', 'POP_DENSITY_SC',
                                  'METRO_INDEX_SC']

            # Calculate the euclidian distance between the selected record and the rest of the dataset
            People_dist = euclidean_distances(filt_master_city.loc[:, PeopleCols_sc],
                                              selected_record[PeopleCols_sc].values.reshape(1, -1))
            Home_dist = euclidean_distances(filt_master_city.loc[:, HomeCols_sc],
                                            selected_record[HomeCols_sc].values.reshape(1, -1))
            Work_dist = euclidean_distances(filt_master_city.loc[:, WorkCols_sc],
                                            selected_record[WorkCols_sc].values.reshape(1, -1))
            Environment_dist = euclidean_distances(filt_master_city.loc[:, EnvironmentCols_sc],
                                                   selected_record[EnvironmentCols_sc].values.reshape(1, -1))

            # Create a new dataframe with the similarity scores and the corresponding index of each record
            df_similarity = pd.DataFrame(
                {'PEOPLE_SIM': People_dist[:, 0], 'HOME_SIM': Home_dist[:, 0], 'WORK_SIM': Work_dist[:, 0],
                 'ENV_SIM': Environment_dist[:, 0], 'index': filt_master_city.index})
            ...
            df_top10 = pd.merge(df_similarity, filt_master_city, left_on='index', right_index=True).reset_index(
                drop=True)
            ...
            with tab2:
                ...
                st.download_button(label="Download Selections as CSV", data=csv, file_name='SIMILO_SAVED.csv',
                                   mime='text/csv', )
            with tab1:
                ...
                fig1 = px.scatter_mapbox(df_top10, lat='LAT', lon='LON',
                                         center=go.layout.mapbox.Center(lat=latcenter, lon=loncenter),
                                         color="Rank", color_continuous_scale=px.colors.sequential.ice,
                                         hover_name='CITYSTATE', hover_data=['Rank'], zoom=3, )
            ...
            if rank_select:
                ...
                with tab1:
                    ...
                    df_long = pd.melt(
                        compare_record[['OVERALL', 'PEOPLE', 'HOME', 'WORK', 'ENVIRONMENT']].reset_index(),
                        id_vars=['index'], var_name='Categories', value_name='Scores')
                ...
                with tab2:
                    ...
                    st.divider()
                    col1, col2 = st.columns(2)
                    fig = px.pie(selected_record,
                                 values=[selected_record['PCT_UNDER_18'][0], selected_record['PCT_18_65'][0],
                                         selected_record['PCT_OVER_65'][0]], names=['< 18', '18-65', '> 65'])
                    fig.update_layout(legend={'title': {'text': 'Age Distribution'}})
                    col1.caption('Selected')
                    col1.plotly_chart(fig, use_container_width=True)
                    fig = px.pie(compare_record,
                                 values=[compare_record['PCT_UNDER_18'][0], compare_record['PCT_18_65'][0],
                                         compare_record['PCT_OVER_65'][0]], names=['< 18', '18-65', '> 65'])
                    fig.update_layout(legend={'title': {'text': 'Age Distribution'}})
                    ...
                    col2.metric('Median Age', compare_record['MED_AGE'][0].round(2),
                                delta=dif_record['MED_AGE'][0].round(2))
                    st.divider()
                    col1, col2 = st.columns(2)
                    col1.caption('Selected')
                    col1.metric('Median Household Income', '${:,.0f}'.format(selected_record['MED_HH_INC'][0].round(2)))
                    col2.caption('Similar')
                    col2.metric('Median Household Income', '${:,.0f}'.format(compare_record['MED_HH_INC'][0].round(2)),
                                delta='${:,.0f}'.format(dif_record['MED_HH_INC'][0].round(2)))
                    st.divider()
                    col1, col2 = st.columns(2)
                    col1.caption('Selected')
                    col1.metric('Percent in Poverty', '{:.1%}'.format(selected_record['PCT_POVERTY'][0].round(2) / 100))
                    col2.caption('Similar')
                    col2.metric('Percent in Poverty', '{:.1%}'.format(compare_record['PCT_POVERTY'][0].round(2) / 100),
                                delta='{:.1%}'.format(dif_record['PCT_POVERTY'][0].round(2) / 100))
                    st.divider()
                    col1, col2 = st.columns(2)
                    col1.caption('Selected')
                    col1.metric('Percent with Bachelors Degree or More',
                                '{:.1%}'.format(selected_record['PCT_BACH_MORE'][0].round(2) / 100))
                    col2.caption('Similar')
                    col2.metric('Percent with Bachelors Degree or More',
                                '{:.1%}'.format(compare_record['PCT_BACH_MORE'][0].round(2) / 100),
                                delta='{:.1%}'.format(dif_record['PCT_BACH_MORE'][0].round(2) / 100))
                with tab3:
                    col1, col2 = st.columns(2)
                    col1.subheader('Selected')
                    col1.write(selected_city)
                    ...
                    col1, col2 = st.columns(2)
                    fig = px.pie(selected_record,
                                 values=[selected_record['PCT_OWN'][0], selected_record['PCT_RENT'][0]],
                                 names=['Percent Own Home', 'Percent Renting'])
                    fig.update_layout(legend={'title': {'text': 'Home Ownership'}})
                    col1.caption('Selected')
                    col1.plotly_chart(fig, use_container_width=True)
                    fig = px.pie(selected_record, values=[compare_record['PCT_OWN'][0], compare_record['PCT_RENT'][0]],
                                 names=['Percent Own Home', 'Percent Renting'])
                    fig.update_layout(legend={'title': {'text': 'Home Ownership'}})
                    col2.caption('Similar')
                    col2.plotly_chart(fig, use_container_width=True)
                    ...
                    col2.caption('Similar')
                    col2.metric('Avg HH Size', '{:,.1f}'.format(compare_record['HH_SIZE'][0].round(2)),
                                delta='{:,.1f}'.format(dif_record['HH_SIZE'][0].round(2)))
                    st.divider()
                    col1, col2 = st.columns(2)
                    col1.caption('Selected')
                    col1.metric('Avg Family Size', '{:,.1f}'.format(selected_record['FAM_SIZE'][0].round(2)))
                    col2.caption('Similar')
                    col2.metric('Avg Family Size', '{:,.1f}'.format(compare_record['FAM_SIZE'][0].round(2)),
                                delta='{:,.1f}'.format(dif_record['FAM_SIZE'][0].round(2)))
                    st.divider()
                    col1, col2 = st.columns(2)
                    col1.caption('Selected')
                    col1.metric('Median Home Price', '${:,.0f}'.format(selected_record['MED_HOME'][0].round(2)))
                    col2.caption('Similar')
                    col2.metric('Median Home Price', '${:,.0f}'.format(compare_record['MED_HOME'][0].round(2)),
                                delta='${:,.0f}'.format(dif_record['MED_HOME'][0].round(2)))
                    st.divider()
                    col1, col2 = st.columns(2)
                    col1.caption('Selected')
                    col1.metric('Median Rent Price', '${:,.0f}'.format(selected_record['MED_RENT'][0].round(2)))
                    col2.caption('Similar')
                    col2.metric('Median Rent Price', '${:,.0f}'.format(compare_record['MED_RENT'][0].round(2)),
                                delta='${:,.0f}'.format(dif_record['MED_RENT'][0].round(2)))
                    st.divider()
                    col1, col2 = st.columns(2)
                    col1.caption('Selected')
                    col1.metric('Pct Single Family Residential',
                                '{:.1%}'.format(selected_record['PCT_UNIT1'][0].round(2) / 100))
                    col2.caption('Similar')
                    col2.metric('Pct Single Family Residential',
                                '{:.1%}'.format(compare_record['PCT_UNIT1'][0].round(2) / 100),
                                delta='{:.1%}'.format(dif_record['PCT_UNIT1'][0].round(2) / 100))
                with tab4:
                    col1, col2 = st.columns(2)
                    col1.subheader('Selected')
                    col1.write(selected_city)
                    ...
                    col1, col2 = st.columns(2)
                    fig = px.pie(selected_record,
                                 values=[selected_record['PCT_SERVICE'][0], selected_record['PCT_BC'][0],
                                         selected_record['PCT_WC'][0]],
                                 names=['Percent Service', 'Percent Blue Collar', 'Percent White Collar'])
                    fig.update_layout(legend={'title': {'text': 'Occupation Type'}})
                    col1.caption('Selected')
                    col1.plotly_chart(fig, use_container_width=True)
                    fig = px.pie(compare_record, values=[compare_record['PCT_SERVICE'][0], compare_record['PCT_BC'][0],
                                                         compare_record['PCT_WC'][0]],
                                 names=['Percent Service', 'Percent Blue Collar', 'Percent White Collar'])
                    fig.update_layout(legend={'title': {'text': 'Occupation Type'}})
                    col2.caption('Similar')
                    col2.plotly_chart(fig, use_container_width=True)
                    ...
                    col2.caption('Similar')
                    col2.metric('Pct Working', '{:.1%}'.format(compare_record['PCT_WORKING'][0].round(2) / 100),
                                delta='{:.1%}'.format(dif_record['PCT_WORKING'][0] / 100))
                    st.divider()
                    col1, col2 = st.columns(2)
                    col1.caption('Selected')
                    ...
                    col2.metric('Avg Commute Time', compare_record['MEAN_COMMUTE'][0].round(2),
                                delta='{:,.1f}'.format(dif_record['MEAN_COMMUTE'][0]))
                with tab5:
                    col1, col2 = st.columns(2)
                    col1.subheader('Selected')
                    col1.write(selected_city)
                    ...
                    col2.caption('Similar')
                    col2.metric('Population Density', '{:,.0f}'.format(compare_record['POP_DENSITY'][0].round(2)),
                                delta='{:.0f}'.format(dif_record['POP_DENSITY'][0]))
                    st.divider()
                    col1, col2 = st.columns(2)
                    col1.caption('Selected')
                    col1.metric('Pct Area is Water', '{:.2%}'.format(selected_record['PCT_WATER'][0]))
                    col2.caption('Similar')
                    col2.metric('Pct Area is Water', '{:.2%}'.format(compare_record['PCT_WATER'][0]),
                                delta='{:.2%}'.format(dif_record['PCT_WATER'][0]))
                    st.divider()
                    col1, col2 = st.columns(2)
                    col1.caption('Selected')
                    col1.metric('Environmental Quality Index',
                                '{:.2f}'.format(selected_record['ENV_INDEX'][0].round(2)))
                    col2.caption('Similar')
                    col2.metric('Environmental Quality Index', '{:.2f}'.format(compare_record['ENV_INDEX'][0].round(2)),
                                delta='{:.2f}'.format(dif_record['ENV_INDEX'][0]))
                    st.divider()
                    col1, col2 = st.columns(2)
                    col1.caption('Selected')
                    col1.metric('Pct within 0.5 mile to Park',
                                '{:.1%}'.format(selected_record['PCT_TOPARK_HALFMILE'][0].round(2) / 100))
                    col2.caption('Similar')
                    col2.metric('Pct within 0.5 mile to Park',
                                '{:.1%}'.format(compare_record['PCT_TOPARK_HALFMILE'][0].round(2) / 100),
                                delta='{:.1%}'.format(dif_record['PCT_TOPARK_HALFMILE'][0] / 100))
                    st.divider()
                    col1, col2 = st.columns(2)
                    col1.caption('Selected')
                    col1.metric('Pct within 1 mile to Park',
                                '{:.1%}'.format(selected_record['PCT_TOPARK_ONEMILE'][0].round(2) / 100))
                    col2.caption('Similar')
                    col2.metric('Pct within 1 mile to Park',
                                '{:.1%}'.format(compare_record['PCT_TOPARK_ONEMILE'][0].round(2) / 100),
                                delta='{:.1%}'.format(dif_record['PCT_TOPARK_ONEMILE'][0] / 100))
    ...
    if zip_select != 'Zip':
        ...
        # Calculate the euclidian distance between the selected record and the rest of the dataset
        People_dist = euclidean_distances(filt_master_zip.loc[:, PeopleCols_sc],
                                          selected_record[PeopleCols_sc].values.reshape(1, -1))
        Home_dist = euclidean_distances(filt_master_zip.loc[:, HomeCols_sc],
                                        selected_record[HomeCols_sc].values.reshape(1, -1))
        Work_dist = euclidean_distances(filt_master_zip.loc[:, WorkCols_sc],
                                        selected_record[WorkCols_sc].values.reshape(1, -1))
        Environment_dist = euclidean_distances(filt_master_zip.loc[:, EnvironmentCols_sc],
                                               selected_record[EnvironmentCols_sc].values.reshape(1, -1))

        # Create a new dataframe with the similarity scores and the corresponding index of each record
        df_similarity = pd.DataFrame(
            {'PEOPLE_SIM': People_dist[:, 0], 'HOME_SIM': Home_dist[:, 0], 'WORK_SIM': Work_dist[:, 0],
             'ENV_SIM': Environment_dist[:, 0], 'index': filt_master_zip.index})
        ...
        df_top10 = df_top10.loc[1:count_select]
        df_top10['RANK'] = list(range(1, count_select + 1))
        df_top10['RANKING'] = df_top10['RANK'].astype(str) + ' - Zip Code ' + df_top10['ZIP'] + ' from ' + df_top10[
            'COUNTY_NAME'] + ' County, ' + df_top10['STATE_SHORT']
        df_top10['LAT_R'] = selected_record['LAT'][0]
        df_top10['LON_R'] = selected_record['LON'][0]
        df_top10['SAVE'] = False
        df_top10['NOTES'] = ''
        ...
        with tab1:
            ...
            fig1 = px.scatter_mapbox(df_top10, lat='LAT', lon='LON',
                                     center=go.layout.mapbox.Center(lat=latcenter, lon=loncenter),
                                     color="RANK", color_continuous_scale=px.colors.sequential.ice, hover_name='ZIP',
                                     hover_data=['RANK', 'COUNTY_NAME'], zoom=3, )
            fig1.update_traces(marker={'size': 15})
            fig1.update_layout(mapbox_style="mapbox://styles/mapbox/satellite-streets-v12",
                               mapbox_accesstoken=token)
            fig1.update_layout(margin={"r": 0, "t": 0, "l": 0, "b": 0})
        ...
        if rank_select:
            ...
            with tab1:
                ...
                col5.metric('Environment', compare_record['ENVIRONMENT'][0].round(2))
                col5.progress(compare_record['ENVIRONMENT'][0] / 100)
                df_long = pd.melt(compare_record[['OVERALL', 'PEOPLE', 'HOME', 'WORK', 'ENVIRONMENT']].reset_index(),
                                  id_vars=['index'], var_name='Categories', value_name='Scores')
                fig = px.bar(df_long, x='Categories', y='Scores', color='Scores', color_continuous_scale='blues')
                fig.update_layout(xaxis_title='Categories',
                                  yaxis_title='Similarity Scores')
            ...
            with tab2:
                ...
                col1.caption('Selected')
                col1.plotly_chart(fig, use_container_width=True)
                fig = px.pie(compare_record, values=[compare_record['PCT_UNDER_18'][0], compare_record['PCT_18_65'][0],
                                                     compare_record['PCT_OVER_65'][0]], names=['< 18', '18-65', '> 65'])
                fig.update_layout(legend={'title': {'text': 'Age Distribution'}})
                col2.caption('Similar')
                col2.plotly_chart(fig, use_container_width=True)
            ...
            with tab3:
                ...
                st.divider()
                col1, col2 = st.columns(2)
                fig = px.pie(selected_record, values=[selected_record['PCT_OWN'][0], selected_record['PCT_RENT'][0]],
                             names=['Percent Own Home', 'Percent Renting'])
                fig.update_layout(legend={'title': {'text': 'Home Ownership'}})
                col1.caption('Selected')
                col1.plotly_chart(fig, use_container_width=True)
            ...
            with tab4:
                ...
                st.divider()
                col1, col2 = st.columns(2)
                fig = px.pie(selected_record, values=[selected_record['PCT_SERVICE'][0], selected_record['PCT_BC'][0],
                                                      selected_record['PCT_WC'][0]],
                             names=['Percent Service', 'Percent Blue Collar', 'Percent White Collar'])
                fig.update_layout(legend={'title': {'text': 'Occupation Type'}})
                col1.caption('Selected')
...
if selected == 'About':
    ...
    with st.container():
        ...
        col1.write(
            '**Contact:**    kevin.soderholm@gmail.com or [linkedin](https://www.linkedin.com/in/kevin-soderholm-67788829/)')
