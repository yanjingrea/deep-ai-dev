...
def fill_nominal_data(num_of_bedrooms, size_rebased: bool = True):
    ...
    filled_bed_data['good_market'] = filled_bed_data['transaction_month'].apply(
...
def calculate_path_revenues(num_of_bedrooms, discount_rate=0.025):
    ...
    for idx, row in bed_data.iterrows():
        ...
        if num_of_bedrooms == 5 and t == 22:
            bed_data.loc[idx, 'price'] = p * 0.8

        if remaining_units > 0:

            raw_q = models[num_of_bedrooms].predict(bed_data.iloc[[idx]]).iloc[0]

            # if num_of_bedrooms in [1, 4]:
            #     good_market = row['good_market']
            #     if good_market:
            #         raw_q
        ...
        filled_bed_data = pd.DataFrame(
            {
                ...
                'num_of_units_launched': [
                                             480, 0, 0, 0, 120, 200, 200, 206, 0, 0, 0, 0, 0, 0, 0, 0, 0
                                         ][:length]
            }
        )
    ...
    def calculate_total_revenue(
        ...
        **kwargs
        ...
        for idx, row in bed_data.iterrows():
            ...
            if self.num_of_bedrooms == 5 and t == 22:
                bed_data.loc[idx, 'price'] = p * 0.8
