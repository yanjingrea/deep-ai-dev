...
class LaunchSalesModel:
    ...
    def fit_and_test_regressor(
        ...
        *,
        ...
        local_x_cols: ListLike = None,
        agg: bool = True,
        return_full_y=False
    ):
        if local_x_cols is None:
            local_x_cols = self.final_x_cols
        ...
        if True:
            ...
            model.fit(
                train[local_x_cols].astype(float),
        ...
        raw_pred_y = model.predict(test[local_x_cols].astype(float))
    ...
    def iter_test(self, *, n_groups=10, test_min_year=None, agg: bool = True):
        ...
        for i, idx_num in enumerate(idx_list):
            ...
            # -------------------------------------------------------------------------------------
            first_stage_data = self.data[self.data['num_of_bedrooms'] == self.first_stage_bed]
            first_stage_res = self.fit_and_test_regressor(
                data=first_stage_data,
                return_full_y=True,
                **common,
            )

            first_stage_pred = pd.Series(
                first_stage_res[self.pred_y_col].values,
                index=first_stage_res[self.project_key]
            )

            # -------------------------------------------------------------------------------------
            second_stage_data = self.data.copy()

            new_col = self.pred_y_col + f'_{self.first_stage_bed}_bed'

            def try_else_na(x):
                try:
                    y = first_stage_pred.loc[x]
                except:
                    return 0

                return y

            second_stage_data[new_col] = second_stage_data[self.project_key].apply(
                lambda a: try_else_na(a)
            )

            res = self.fit_and_test_regressor(
                data=second_stage_data,
                local_x_cols=np.append(self.final_x_cols, new_col),
                return_full_y=False,
                **common,
            )

            # res = pd.concat([first_stage_res, second_stage_res], ignore_index=True)
...
LaunchSalesModel(
    ...
    y_col='sales_rate',
    first_stage_bed=4
).evaluate(test_min_year=2019, agg=False)
